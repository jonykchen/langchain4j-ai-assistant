...
public class DocumentController {
    ...
    public ApiResponse<DocumentInfo> uploadDocument(@RequestParam("file") MultipartFile file) {
        try {
            Document document = ragPipeline.ingest(file.getInputStream(), file.getOriginalFilename());

            return ApiResponse.success(new DocumentInfo(
                    document.id(),
                    document.filename(),
                    document.type().name(),
                    document.fileSize(),
    ...
    public ApiResponse<List<SearchResultInfo>> search(@RequestBody SearchRequest request) {
        List<VectorStore.SearchResult> results = ragPipeline.retrieveWithScore(
                request.query(),
                request.topK() != null ? request.topK() : 5
        );

        List<SearchResultInfo> infos = results.stream()
                .map(this::toSearchResultInfo)
                .toList();
    ...
    public ApiResponse<RAGResponseInfo> ask(@RequestBody AskRequest request) {
        RAGPipeline.RAGResponse response = ragPipeline.askWithDetails(
                request.query(),
                request.topK() != null ? request.topK() : 5
        );

        RAGResponseInfo info = new RAGResponseInfo(
                response.answer(),
                response.sources().stream()
                        .map(s -> new SourceInfo(s.documentId(), s.filename(), s.content(), s.relevance()))
                        .toList()
        );
