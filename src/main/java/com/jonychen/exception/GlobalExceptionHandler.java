 ...
 * 全局异常处理器
 *
 * <p>统一处理所有 Controller 抛出的异常，返回标准化的错误响应格式
 *
 * <p>异常处理优先级： 1. BusinessException - 业务异常，返回具体错误码和消息 2. 参数校验异常 - 返回参数错误提示 3. 未知异常 -
 * 返回通用服务器错误，记录详细日志
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleMissingParamException(
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(
                        ApiResponse.error(
                                ErrorCode.PARAM_MISSING, "缺少必需参数: " + e.getParameterName()));
    }
