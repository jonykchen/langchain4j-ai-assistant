 ...
 * 聊天控制器
 *
 * <p>Resilience4j 容错能力： - @RateLimiter: 限流，防止 API 滥用 - @CircuitBreaker: 熔断，AI 服务异常时快速失败 - @Retry:
 * 重试，网络抖动时自动重试
 *
 * @author 30240
 */
...
public class ChatController {
     ...
     * 同步对话接口
     *
     * <p>容错配置： - 限流：每分钟 20 次（开发环境 100 次） - 熔断：失败率 30% 触发熔断，等待 30 秒后半开 - 重试：失败后最多重试 2 次
     */
    @PostMapping
     ...
     *
     * <p>容错配置： - 限流：每分钟 30 次（开发环境 150 次） - 熔断：失败率 30% 触发熔断
     */
    ...
    public Flux<ServerSentEvent<String>> chatStreamFallback(
            ChatRequest request, io.github.resilience4j.ratelimiter.RequestNotPermitted e) {
        return Flux.just(
                ServerSentEvent.<String>builder().event("error").data("请求过于频繁，请稍后再试").build());
    }
    ...
    public Flux<ServerSentEvent<String>> chatStreamCircuitBreakerFallback(
            ...
            io.github.resilience4j.circuitbreaker.CallNotPermittedException e) {
        return Flux.just(
                ServerSentEvent.<String>builder().event("error").data("AI 服务暂时不可用，请稍后再试").build());
    }
}
