 ...
 * 多模型配置属性
 *
 * 从 application.properties 读取模型提供者配置，格式：
 * model.providers.{name}.base-url=xxx
 * model.providers.{name}.model-name=xxx
 * model.providers.{name}.weight=xxx
 * model.providers.{name}.priority=xxx
 * model.providers.{name}.enabled=xxx
 *
 * API Key 通过环境变量注入：
 * model.providers.{name}.api-key=${ENV_VAR:}
 */
@Configuration
...
public class ModelProperties {
    ...
    /**
     * 模型提供者配置映射
     * key: 提供者名称（dashscope, zhipu, deepseek 等）
     * value: 提供者配置
     */
    ...
    public void setProviders(Map<String, ProviderConfig> providers) {
        ...
        providers.forEach((name, config) -> {
            LOG.info("  - {}: enabled={}, hasApiKey={}, baseUrl={}",
                    name,
                    config.enabled,
    ...
    public List<ModelProvider> getEnabledProviders() {
        ...
        providers.forEach((name, config) -> {
            LOG.debug("  检查 {}: enabled={}, hasApiKey={}, baseUrl={}, modelName={}",
                    name, config.enabled,
                    config.apiKey != null && !config.apiKey.isBlank(),
            ...
            if (config.enabled != null && config.enabled) {
                ModelProvider provider = new ModelProvider(
                        ...
                        config.priority != null ? config.priority : Integer.MAX_VALUE,
                        true
                );
                if (provider.isValid()) {
                    result.add(provider);
                    LOG.info("有效模型: {} (优先级={}, 权重={})", name, provider.priority(), provider.weight());
                } else {
                    LOG.warn("无效模型 {}: hasApiKey={}, hasBaseUrl={}, hasModelName={}",
                            name,
                            config.apiKey != null && !config.apiKey.isBlank(),
