...
public class CalculatorTools {
    ...
    /** 基础数学计算 */
    @AgentTool(name = "calculate", description = "执行基础数学计算：加减乘除", category = ToolCategory.SYSTEM)
    public ToolResult calculate(
        ...
        try {
            ...
            return ToolResult.success(
                    Map.of("result", formatResult(result), "expression", expression));
    ...
    public ToolResult calculatePercentage(
            ...
            @ToolParam(name = "percentage", description = "百分比值，如 20 表示 20%") Double percentage,
            @ToolParam(
                            name = "operation",
                            description = "操作类型：of(求百分比值)、add(增加百分比)、subtract(减少百分比)")
                    String operation) {
        try {
            ...
            return ToolResult.success(
                    Map.of("result", formatResult(result), "description", description));
        } catch (Exception e) {
    ...
    public ToolResult convertUnits(
            @ToolParam(name = "value", description = "数值") Double value,
            @ToolParam(name = "from_unit", description = "原单位，如 km, kg, celsius") String fromUnit,
        ...
        try {
            ...
            Map<String, Object> result =
                    convert(value, fromUnit.toLowerCase(), toUnit.toLowerCase());
    ...
    private Map<String, Object> convert(double value, String from, String to) {
        ...
        Map<String, Double> lengthUnits =
                Map.ofEntries(
                        ...
                        Map.entry("cm", 0.01),
                        Map.entry("centimeter", 0.01),
                        Map.entry("centimeters", 0.01),
                        Map.entry("mm", 0.001),
                        Map.entry("millimeter", 0.001),
                        Map.entry("millimeters", 0.001),
                        ...
                        Map.entry("inches", 0.0254));

        // 重量转换（基准：千克）
        Map<String, Double> weightUnits =
                Map.ofEntries(
                        Map.entry("kg", 1.0),
                        Map.entry("kilogram", 1.0),
                        Map.entry("kilograms", 1.0),
                        Map.entry("g", 0.001),
                        Map.entry("gram", 0.001),
                        Map.entry("grams", 0.001),
                        Map.entry("lb", 0.453592),
                        Map.entry("pound", 0.453592),
                        Map.entry("pounds", 0.453592),
                        Map.entry("oz", 0.0283495),
                        Map.entry("ounce", 0.0283495),
                        Map.entry("ounces", 0.0283495));
