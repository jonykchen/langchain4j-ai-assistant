...
public record ActionPlan(
    ...
    public static ActionPlan simple(String goal, String action, String tool) {
        return new ActionPlan(
                goal,
                List.of(new Step(1, action, tool, Map.of(), action, "成功执行", 5, StepStatus.PENDING.getValue())),
                5,
    ...
    public ActionPlan addStep(Step step) {
        ...
        newSteps.add(step);
        return new ActionPlan(goal, newSteps, estimatedTime + step.estimatedDuration(),
                complexity, dependencies, fallbackPlan);
    }
}