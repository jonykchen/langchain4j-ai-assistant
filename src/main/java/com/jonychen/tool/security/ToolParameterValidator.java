...
public class ToolParameterValidator {
    ...
    private static final List<Pattern> SQL_INJECTION_PATTERNS =
            List.of(
                    Pattern.compile(
                            "(?i)(drop|delete|truncate|alter|create|exec|execute)\\s+",
    ...
    private static final List<Pattern> XSS_PATTERNS =
            List.of(
                    ...
                    Pattern.compile(
                            "on(load|error|click|mouseover|focus|blur)\\s*=",
                            Pattern.CASE_INSENSITIVE),
    ...
    private List<String> validateBusinessRules(Map<String, Object> params, ToolDefinition toolDef) {
        ...
        for (Map.Entry<String, ToolParameterSchema.Property> entry :
            ...
            if (prop.getEnumValues() != null
                    && !prop.getEnumValues().isEmpty()
                    && value instanceof String) {
                if (!prop.getEnumValues().contains((String) value)) {
                    errors.add(
                            String.format("参数 '%s' 值无效: 可选值为 %s", paramName, prop.getEnumValues()));
                }
            }
    ...
    private boolean containsSqlInjection(String str) {
        ...
        return SQL_INJECTION_PATTERNS.stream().anyMatch(pattern -> pattern.matcher(str).find());
    }
    ...
    private boolean containsXss(String str) {
        ...
        return XSS_PATTERNS.stream().anyMatch(pattern -> pattern.matcher(str).find());
    }
}
