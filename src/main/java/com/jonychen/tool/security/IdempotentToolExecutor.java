...
public class IdempotentToolExecutor {
    ...
    /** 需要幂等性控制工具分类 */
    private static final Set<ToolCategory> IDEMPOTENT_CATEGORIES =
            Set.of(
    ...
    /** 带幂等性控制的执行 */
    private ToolResult executeWithIdempotency(
            ToolDefinition tool, Map<String, Object> params, String sessionId) {
        String idempotencyKey =
                idempotencyManager.generateIdempotencyKey(tool.name(), params, sessionId);

        // 检查是否已执行
        if (!idempotencyManager.checkAndSet(idempotencyKey)) {
            // 尝试获取缓存结果
            return idempotencyManager
                    .getCachedResult(idempotencyKey)
    ...
    public ToolResult forceExecute(String toolName, Map<String, Object> params, String sessionId) {
        ...
        // 释放幂等键后执行
        String idempotencyKey =
                idempotencyManager.generateIdempotencyKey(toolName, params, sessionId);
        idempotencyManager.release(idempotencyKey);
