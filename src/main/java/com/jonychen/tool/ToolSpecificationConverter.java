...
/**
 * ToolSpecification 转换器 将内部工具定义转换为 LangChain4j 格式
 *
 * @author jonychen
 */
public class ToolSpecificationConverter {
    ...
    public static ToolSpecification convert(ToolDefinition definition) {
        ...
        if (schema != null && schema.getProperties() != null && !schema.getProperties().isEmpty()) {
            ...
            for (Map.Entry<String, ToolParameterSchema.Property> entry :
    ...
    private static JsonSchemaElement convertProperty(ToolParameterSchema.Property prop) {
        return switch (prop.getType().toLowerCase()) {
            case "string" -> JsonStringSchema.builder().description(prop.getDescription()).build();
            case "integer" ->
                    JsonIntegerSchema.builder().description(prop.getDescription()).build();
            case "number" -> JsonNumberSchema.builder().description(prop.getDescription()).build();
            case "boolean" ->
                    JsonBooleanSchema.builder().description(prop.getDescription()).build();
            case "array" ->
                    JsonArraySchema.builder()
                            .description(prop.getDescription())
                            .items(JsonStringSchema.builder().build())
                            .build();
            case "object" -> JsonObjectSchema.builder().description(prop.getDescription()).build();
            default -> JsonStringSchema.builder().description(prop.getDescription()).build();
        };
    }
}
