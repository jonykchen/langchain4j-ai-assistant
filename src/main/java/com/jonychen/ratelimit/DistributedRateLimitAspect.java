 ...
 * 分布式限流切面
 *
 * 基于 Redis 实现全局限流，多实例共享配额
 * 支持从 Nacos 配置中心动态更新限流参数
 */
@Aspect
@Component
public class DistributedRateLimitAspect {
    ...
    public DistributedRateLimitAspect(DistributedRateLimiter rateLimiter, RateLimitProperties rateLimitProperties) {
    ...
    public Object around(ProceedingJoinPoint point) throws Throwable {
        ...
        if (!allowed) {
            log.warn("分布式限流拒绝: method={}, key={}, limit={}, period={}s", method.getName(), key, limit, period);
            throw new BusinessException(ErrorCode.RATE_LIMITED,
                    String.format("请求过于频繁，请在 %d 秒后重试", period));
        }
