...
public class UserAdminService {
     ...
     * 分页获取用户列表
     */
    public PageResponse<UserAdminVO> getUsers(int page, int size, String search, String role, String provider) {
        ...
        if (search != null && !search.isBlank()) {
            ...
            userPage = userRepository.findByProvider(com.jonychen.auth.AuthProvider.valueOf(provider), pageable);
        } else {
            userPage = userRepository.findAll(pageable);
        }

        List<UserAdminVO> users = userPage.getContent().stream()
    ...
    public void updateUserQuota(String userId, QuotaUpdateRequest request) {
        ...
        // 存储配额到用户元数据（简化实现）
        // 实际项目中可以创建单独的配额表
        log.info("Updated user {} quota: daily={}, monthly={}",
                userId, request.dailyTokenLimit(), request.monthlyTokenLimit());
    }
    ...
    public UserUsageStats getUserUsageStats(String userId) {
        ...
        long todayTokens = todayUsage.isEmpty() || todayUsage.get(0)[0] == null ? 0 :
                ((Number) todayUsage.get(0)[0]).longValue();
        double todayCost = todayUsage.isEmpty() || todayUsage.get(0)[1] == null ? 0 :
                ((Number) todayUsage.get(0)[1]).doubleValue();

        long monthTokens = monthUsage.isEmpty() || monthUsage.get(0)[0] == null ? 0 :
                ((Number) monthUsage.get(0)[0]).longValue();
        double monthCost = monthUsage.isEmpty() || monthUsage.get(0)[1] == null ? 0 :
    ...
    private UserAdminVO toUserAdminVO(User user) {
        ...
        long todayTokens = usage.isEmpty() || usage.get(0)[0] == null ? 0 :
                ((Number) usage.get(0)[0]).longValue();
        double todayCost = usage.isEmpty() || usage.get(0)[1] == null ? 0 :
