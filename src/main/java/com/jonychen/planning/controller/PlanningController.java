...
public class PlanningController {
    ...
    @PostMapping("/task")
    public ApiResponse<TaskResultInfo> executePredefinedTask(@RequestBody PredefinedTaskRequest request) {
        ...
        for (int i = 0; i < request.steps().size(); i++) {
            ...
            Step step = stepDef.tool() != null
                    ? Step.createWithTool(i + 1, stepDef.description(), stepDef.tool(), stepDef.params())
    ...
    public record PredefinedTaskRequest(
            String goal,
            String sessionId,
            List<StepDef> steps
    ) {}
