...
test.describe('任务规划页面测试', () => {
  ...
  test.describe('页面基础功能', () => {
    ...
    test('策略切换', async ({ page }) => {
      ...
      await expect(page.locator('.el-radio-button:has-text("ReAct")')).toHaveAttribute(
        'aria-checked',
        'true'
      )
      ...
      await expect(page.locator('.el-radio-button:has-text("Plan-Execute")')).toHaveAttribute(
        'aria-checked',
        'true'
      )
      ...
      await expect(page.locator('.el-radio-button:has-text("预定义任务")')).toHaveAttribute(
        'aria-checked',
        'true'
      )
  ...
  test.describe('步骤构建器测试', () => {
    ...
    test('切换步骤类型', async ({ page }) => {
      ...
      // 验证动作模式选中
      await expect(stepCard.locator('.el-radio[value="action"]')).toHaveAttribute(
        'aria-checked',
        'true'
      )
      ...
      // 验证工具模式选中
      await expect(stepCard.locator('.el-radio[value="tool"]')).toHaveAttribute(
        'aria-checked',
        'true'
      )
  ...
  test.describe('统计卡片测试', () => {
    test('执行后统计更新', async ({ page }) => {
      ...
      const initialCount = await page
        .locator('.el-statistic .el-statistic__number')
        .first()
        .textContent()
