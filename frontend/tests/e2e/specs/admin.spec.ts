    ...
    await page.route('**/api/admin/users**', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          data: {
            data: [
              { id: '1', username: 'testuser', email: 'test@example.com', nickname: '测试用户', role: 'USER', provider: 'CUSTOM', createdAt: '2024-01-01', lastLoginAt: '2024-01-15', todayTokens: 1000, todayCost: 0.5 },
              { id: '2', username: 'adminuser', email: 'admin@example.com', nickname: '管理员', role: 'ADMIN', provider: 'CUSTOM', createdAt: '2024-01-02', lastLoginAt: '2024-01-16', todayTokens: 2000, todayCost: 1.0 },
            ],
            total: 2,
            page: 1,
            size: 10,
    ...
    await page.route('**/api/admin/cost/model-statistics**', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          message: 'success',
          data: [
            { modelName: 'qwen-plus', totalTokens: 100000, promptTokens: 50000, completionTokens: 50000, totalCost: 15.5, requestCount: 500, avgTokensPerRequest: 200, costPercent: 60 },
          ]
        })
      });
    });
...
test.describe('管理员权限测试', () => {
  test('普通用户无法访问管理后台', async ({ page }) => {
    ...
    // 验证被拒绝访问或重定向
    const url = page.url();
    const hasAccessDenied = await page.locator('.access-denied, .forbidden, [data-testid="access-denied"]').isVisible().catch(() => false);
