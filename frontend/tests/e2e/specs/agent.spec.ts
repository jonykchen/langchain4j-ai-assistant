...
test.describe('Agent 执行功能', () => {
  ...
  test.describe('Agent 选择', () => {
    ...
    test('选择 Agent 后应显示描述信息', async ({ authenticatedPage }) => {
      ...
      // 验证描述信息显示
      await expect(authenticatedPage.locator('[data-testid="agent-description"]')).toContainText('智能路由');
    });
  });
  ...
  test.describe('Agent 执行', () => {
    ...
    test('执行 Agent 并显示步骤', async ({ authenticatedPage }) => {
      ...
      await authenticatedPage.route('**/api/agent/execute', async (route) => {
        const events = [
          { eventType: 'step_start', stepIndex: 0, type: 'THOUGHT', agentName: 'router', sequenceNumber: 1 },
          { eventType: 'thought', stepIndex: 0, content: '分析用户输入...', sequenceNumber: 2 },
          { eventType: 'step_end', stepIndex: 0, success: true, summary: '已分析', sequenceNumber: 3 },
          { eventType: 'step_start', stepIndex: 1, type: 'TOOL_CALL', agentName: 'router', sequenceNumber: 4 },
          { eventType: 'tool_call', stepIndex: 1, toolName: 'classify_intent', sequenceNumber: 5 },
          { eventType: 'tool_result', stepIndex: 1, toolName: 'classify_intent', success: true, sequenceNumber: 6 },
          { eventType: 'step_end', stepIndex: 1, success: true, summary: '意图分类完成', sequenceNumber: 7 },
          { eventType: 'agent_done', agentName: 'router', output: '已选择 DataAgent', totalSteps: 2, sequenceNumber: 8 }
        ];
      ...
      // 等待执行步骤显示
      await authenticatedPage.locator('[data-testid="execution-step"]').first().waitFor({ timeout: 30000 });
    ...
    test('执行完成后应显示统计信息', async ({ authenticatedPage }) => {
      ...
      await authenticatedPage.route('**/api/agent/execute', async (route) => {
        const events = [
          ...
          { eventType: 'step_end', stepIndex: 0, success: true, summary: '完成', sequenceNumber: 3 },
          {
    ...
    test('执行错误应显示错误信息', async ({ authenticatedPage }) => {
      ...
      // 验证错误信息
      await expect(authenticatedPage.locator('[data-testid="error-message"]')).toContainText('执行失败');
    });
  });
  ...
  test.describe('敏感操作确认', () => {
    ...
    test('敏感操作应显示确认对话框', async ({ authenticatedPage }) => {
      ...
      await authenticatedPage.route('**/api/agent/execute', async (route) => {
        const events = [
          { eventType: 'step_start', stepIndex: 0, type: 'TOOL_CALL', agentName: 'ops', sequenceNumber: 1 },
          {
      ...
      // 等待确认对话框显示
      await authenticatedPage.locator('[data-testid="confirmation-dialog"]').waitFor({ timeout: 30000 });

      // 验证对话框内容
      await expect(authenticatedPage.locator('[data-testid="confirmation-operation"]')).toContainText('restart_service');
      await expect(authenticatedPage.locator('[data-testid="confirmation-risk-level"]')).toContainText('HIGH');
    });
    ...
    test('批准确认后继续执行', async ({ authenticatedPage }) => {
      ...
      await authenticatedPage.route('**/api/agent/execute', async (route) => {
        ...
        if (callCount === 1) {
          ...
          // 第二阶段：执行完成
          const events = [
            { eventType: 'agent_done', agentName: 'ops', output: '部署成功', totalSteps: 2, sequenceNumber: 2 }
          ];
          const body = events.map(e =>
            `event:${e.eventType}\ndata:${JSON.stringify(e)}\n\n`
          ).join('') + 'event:done\ndata:[DONE]\n\n';
      ...
      // 等待执行完成
      await authenticatedPage.locator('[data-testid="execution-output"]').waitFor({ timeout: 30000 });

      // 验证输出
      await expect(authenticatedPage.locator('[data-testid="execution-output"]')).toContainText('部署成功');
    });
  });
