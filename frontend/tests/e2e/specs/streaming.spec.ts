...
test.describe('流式响应测试', () => {
  ...
  test('流式响应包含代码块', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.locator('[data-testid="assistant-message"] pre, [data-testid="assistant-message"] code')).toBeVisible();
  });
  ...
  test('流式响应中途断开', async ({ authenticatedPage }) => {
    ...
    // 验证部分消息已显示
    await authenticatedPage.locator('[data-testid="assistant-message"]').waitFor({ timeout: 30000 });
    await expect(authenticatedPage.locator('[data-testid="assistant-message"]')).toContainText('部分响应');
  });
  ...
  test('流式响应错误事件', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.locator('[data-testid="assistant-message"]')).toContainText('AI 服务');
  });
  ...
  test('多个流式请求顺序处理', async ({ authenticatedPage }) => {
    ...
    const count = await authenticatedPage.locator('[data-testid^="message-"], [data-testid="user-message"], [data-testid="assistant-message"]').count();
