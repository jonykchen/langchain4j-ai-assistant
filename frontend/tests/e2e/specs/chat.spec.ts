...
test.describe('聊天功能', () => {
  ...
  test('流式响应正确显示', async ({ authenticatedPage }) => {
    ...
    // 验证消息逐步显示
    const messageText = await authenticatedPage
  ...
  test('思考过程可折叠显示', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.locator('[data-testid="thinking-section"]')).toContainText(
      '正在思考'
    )
  })

  test('代码块可以复制', async ({ authenticatedPage }) => {
    await chatMock.mockChatWithCode('java', 'System.out.println("Hello");', '这是一个 Java 示例')
    ...
    // 验证消息内容包含 Java 和 示例
    const messageContent = await authenticatedPage
      .locator('[data-testid="assistant-message"]')
      .textContent()
  ...
  test('连续发送多条消息', async ({ authenticatedPage }) => {
    ...
    await authenticatedPage.locator('[data-testid="assistant-message"]').waitFor({ timeout: 30000 })
    await expect(
      authenticatedPage.locator('[data-testid="assistant-message"]').first()
    ).toContainText('第一条')
    ...
    await expect(
      authenticatedPage.locator('[data-testid="assistant-message"]').nth(1)
    ).toContainText('第二条')
  })
  ...
  test('空消息不能发送', async ({ authenticatedPage }) => {
    ...
    const assistantMessageCount = await authenticatedPage
      .locator('[data-testid="assistant-message"]')
      .count()
    // 初始状态不应该有任何消息
